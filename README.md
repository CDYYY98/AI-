...
桌面客户端更新到新的正式发布包，移动端导航和设置入口补齐：安装版用户后续可以通过应用内检查更新获取本次修复，移动端也能继续在导航区域切换模型并进入知识库设置。

- 移动端导航重新显示紧凑模型选择器，自动导演和创作页面不用退回桌面布局也能确认当前模型。
- 移动端更多菜单保留后台和管理入口，管理员在窄屏设备上也能继续检查模型、卡密和调用配置。
- 设置页的知识库入口在移动端布局中更稳定，按钮不会挤压或错位。
- 桌面安装版发布版本推进到 `0.3.20`，用于 GitHub Release 和客户端检查更新识别新版。
