...
章节生成后的结构化审校、自动导演接管状态、运行展示、用量保护和修复兜底更稳：系统会更明确约束 AI 输出格式，更准确地区分真正的自动导演任务和普通编辑任务，并按当前执行范围判断章节审校与修复进度。
...
- 差评值、评分、倒计时等数值状态会按连续性记录需要保存为可读状态文本，减少因为字段类型不一致触发修复或中断。
- 自动导演会按当前剩余章节范围统计单章 AI 用量，异常超过预算时暂停继续执行，避免范围外旧章节误触发熔断，也避免同一章反复修复造成额度失控。
- 局部修文会更严格要求可唯一定位的正文片段；遇到过短片段、不可安全应用的补丁或接收闸门暂时不可用时，会保留正文并进入可恢复复查，不再用错误补丁硬改正文。
- 自动导演仍在运行或排队时，章节标题提醒不会覆盖主进度面板，用户能继续看到当前执行进度和主要动作。
- 自动导演只执行部分章节时，章节审校和质量修复会按当前范围统计，范围外旧章节不会再误判当前批次还没完成或仍需修复。
- 自动导演停在确认点时，小说页和工作台会继续刷新状态；章节执行中的驾驶舱标题也会优先显示“正在审校/生成第 N 章”等具体进度。
- 伏笔账本会复用同义未完成条目；没有明确目标章节窗口的逾期伏笔会作为待跟进风险继续推进，不再轻易把自动导演卡进重规划。
