...
桌面客户端发布和更新入口切换到自有 GitHub 仓库：正式用户下载新版、客户端检查更新、发布工作流上传安装包时，都会使用当前项目自己的 Release 通道。

- Windows 桌面版下载入口指向 `CDYYY98/AI-` 的 GitHub Releases，用户不需要再进入上游开源仓库查找安装包。
- 安装版客户端写入的更新源默认使用自有仓库，应用内“检查更新”会从同一发布通道获取新版。
- 更新入口会按实际通道显示“正式 / 测试”，发布通道不可访问时会提示处理方向，不再展示大段技术报错。
- GitHub Actions 的桌面正式版和测试版发布目标统一改为自有仓库，后续发版不再依赖上游仓库写入权限。
