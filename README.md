...
章节摘要会额外提取正文里已经写明的硬事实，例如承诺、交易条款、私下或公开性质、关键数字、日期和身份变化；这些事实会进入后续上下文和检索材料，减少后续章节把已经确定的细节写成矛盾版本。

- 章节摘要仍兼容原来的摘要格式；没有明确硬事实的章节会正常生成摘要，不会强行补设定。
- 后续章节更容易记住本章已经确定的具体事实，例如某件事是私下约定、金额数量是多少、关系或状态发生了什么变化。
