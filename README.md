...
桌面客户端发布链路面向正式用户收敛到 `Setup.exe` 安装版：用户安装后可在系统设置里检查更新，下载完成后重启应用即可安装新版。

- Windows 默认打包目标只产出安装版，降低用户在安装版和便携版之间选错的概率。
- 个人中心显示“软件更新”入口，普通用户可以查看当前版本、更新状态、可用版本，并手动检查更新。
- 个人中心的卡密购买入口改为链动小铺店铺页，后续新增不同面额卡密时不需要频繁更换客户端入口。
- 桌面客户端推进到 `0.3.17`，安装包会连接正式服务器账号与邮箱验证码服务，避免注册时落到本地内置服务而无法发送验证码。
- 正式安装包增加专用打包入口，后续生成 `Setup.exe` 时会自动写入正式 API 地址，减少打包环境漏配导致的注册、卡密和账户数据异常。
- 安装包文件名与更新清单统一使用稳定英文文件名，应用内检查更新时可以按同一个安装包地址下载新版。
- 更新包下载完成后会直接弹出“安装新版”确认框，用户可以马上安装，也可以稍后在个人中心继续点击“重启安装”。
