...
角色准备、章节生产、卷规划同步、后台副作用、桌面打包和全书自动执行链路更稳：用户在角色准备页应用一套阵容后，核心角色、关系和方案状态会先同步完成；卷规划同步到章节执行区时，会记住每个规划章节对应的真实执行章节；自动导演遇到需要确认的角色阵容候选时，会停在可处理的审核点；全书自动执行遇到重复质量问题时，也能更准确暂存当前章节风险并继续推进；空白章节会按可重新写作的状态提示处理，已登记为质量待回收的章节也能继续后续章节。

- 应用角色阵容后会提示外显资料和角色动态正在后台补齐，用户可以先继续进入角色资产工作台。
- 后台补齐会沿用当前选择的模型参数，减少应用阵容时长时间等待或误以为页面卡住的情况。
- 自动导演内部链路仍会等待增强补齐完成，避免后续卷策略或结构化大纲读取到不完整的角色动态。
- 角色阵容候选会保留给用户审核或应用，任务详情会围绕“等待确认”继续引导处理。
- 真正缺少角色、候选和恢复检查点时，系统才会把角色准备视为失败。
- 首页和侧边栏会延后加载非关键徽标、知识库状态和恢复任务提示，减少刚打开应用时同时发起的请求。
- 小说列表改为分页加载并显示当前页数和总数，小说较多时首屏不会一次拉取过多项目。
- AI 自动导演进度弹窗会稳定保留流程步骤，不再因为后台轮询刷新后退化成只有全部进展和里程碑历史的视图。
- 自动导演停在重规划、角色审核等确认点时，会优先显示等待处理的检查点，不再被过期的后台运行步骤覆盖成仍在执行。
- 全书自动执行把重复重规划或修复失败暂存为质量债时，会按实际出问题的章节记录，不会因为状态刷新把下一章误标为跳过章节。
- 章节审校发现局部计划问题时，会先作为质量提醒和局部修复继续处理，不再轻易升级成整段重规划，减少后续章节被误阻塞的情况。
- 卷规划同步到章节执行区时，会优先沿用已经绑定的执行章节；即使章节重名、顺序移动或旧标题重复，也不会只靠标题和序号把规划同步到错误章节。
- 新建执行章节后会把真实章节链接写回卷工作区和版本记录，后续继续同步时能沿用同一章，减少重复创建、误覆盖已有正文或把章节资产挂错位置的风险。
- 章节生成、卷规划保存和整段执行完成后的角色动态同步、卷角色重建、里程碑快照会进入后台队列执行；长链路不再因为这些隐藏副作用被同步拖慢，服务重启后也能继续重试未完成的后台工作。
- 没有正文的章节即使曾经进入生成状态，也会提示重新写正文，不会只因为后台状态残留就显示为正在执行。
- 已明确登记为“质量待回收并继续”的章节，不会再因为缺少额外状态快照被误判为待提交或待修复，自动导演可以继续推进后续章节。
- GitHub Actions 桌面打包流程改用 Node 24 运行环境验证，提前适配新版 Actions 运行时，减少后续生成安装包时因 Node 版本切换失败的风险。
