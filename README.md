...
后台模型和账户能力更适合正式运营：管理员可以按账户类型分别配置体验账户、创作账户使用的模型，让不同用户群使用不同成本和能力的模型组合。

- 模型配置页增加账户类型模型路由入口，后台接口会保存体验账户和创作账户各自的模型选择，普通创作请求会按用户账户类型自动取用对应模型。
- 灵感值、NewAPI 额度和用户账户状态的展示链路更统一，方便排查用户余额、充值后额度同步和模型调用成本。
- 个人中心支持卡密兑换，用户拿到卡密后可以自助补充灵感值；后台可以批量生成、导出、停用和查看卡密使用状态，减少人工守着电脑加额度的压力。
- 模型调用兼容更多 NewAPI 上游通道；遇到不支持 `developer` 角色的通道时，系统会用可接受的消息格式请求模型，正常返回结果后再同步扣减灵感值。
- 注册流程补齐验证码发送的开发测试开关；正式环境可以关闭调试验证码并配置真实 SMTP，测试环境可以直接看到验证码，降低打包验收时的阻塞。
- 管理后台补齐用户、日志和模型相关入口，方便从后台查看用户状态、处理测试账户和核对模型配置。
- Windows 桌面包构建流程补齐本地运行所需资源整理，便于生成可直接测试的便携版客户端。
