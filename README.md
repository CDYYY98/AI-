...
自动导演章节执行、章节状态判断、开书起步和首屏任务提醒更清楚：当某一章没有生成出可保存正文时，任务详情会显示真实中断原因；当只自动执行部分章节时，章节状态提交会按当前执行范围判断，避免范围外旧章节拖住本轮进度；新手没有明确想法时，也可以先让自动导演给出 5 个临时起始灵感；首页和任务提醒会用更轻的摘要读取，减少刚打开应用时的等待。
