...
- 下载入口：[GitHub Releases](https://github.com/CDYYY98/AI-/releases)
- 最新版本页：[Latest Release](https://github.com/CDYYY98/AI-/releases/latest)
- 正式用户统一下载 `Setup.exe` 安装版；安装版支持在应用内检查并安装后续更新
- `portable` 便携版仅作为临时测试或特殊场景备用，不作为正式用户的推荐下载方式
...
自动导演章节执行、章节状态判断、开书起步、首屏任务提醒、本地日志维护和章节生成质量更清楚：当某一章没有生成出可保存正文时，任务详情会显示真实中断原因；当只自动执行部分章节时，章节状态提交会按当前执行范围判断，避免范围外旧章节拖住本轮进度；全书自动接管会正确识别“章节任务单稍后自动生成”的推进方式；写章上下文可以明确提醒已完成目标、近期重复场景和卷级关键节点，减少反复追求同一目标或提前写完卷高潮；章节质量债务会记录失败根因并支持汇总分析，帮助判断问题来自计划错位、修复未闭环、patch 锚点失配还是长度与内容修复漂移；新手没有明确想法时，也可以先让自动导演给出 5 个临时起始灵感；首页和任务提醒会用更轻的摘要读取，章节接收检查也会复用可确认的成功结果，手动章节生成也会沿用统一生产链，减少重复等待和入口差异；服务端和桌面端会自动清理过期日志并轮转超大日志，减少长期运行占用磁盘。

- 章节执行没有产出正文时，会把任务里的失败信息或检查点摘要带到校验结果里，不再只显示泛泛的“没有可观察正文”。
- 自动导演只执行指定章节范围时，章节状态提交会只统计当前范围内的正文和状态提交结果；范围外未提交的旧章节不会再让本轮章节链路误显示未完成。
- 全书自动接管下，章节任务单可以在进入章节执行前自动生成；系统不会再因为这一步尚未提前全量细化，就把结构化大纲阶段误判为失败。
- 章节写作上下文支持“已完成目标”“近期场景模式黑名单”和“卷级关键节点守卫”，让 AI 写章时更容易避开重复办同一件事、复用同一开场模式或提前写完后续卷高潮。
- 章节质量债务会记录首次失败、二次失败、patch 锚点、计划错位和长度/内容漂移等根因，并可通过只读分析工具汇总，方便判断后续应优化章节计划、修复器还是局部 patch。
- 自动导演开书输入框增加“没有想法？”入口，会按当前标题、题材、世界观、书级卖点和写法偏好生成 5 个不同方向的起始灵感；灵感只作为临时参考，用户选择后才会填入输入框，不会自动保存或直接参与生成。
- 待恢复任务和自动导演跟进提醒会优先读取轻量摘要，减少打开首页、侧边栏或任务提醒时反复组装完整任务详情；真正进入详情或执行恢复时，系统再做必要的状态校正。
- 同一章正文和同一模型参数已经通过接收检查时，后续重试或服务重启后的继续执行会优先复用成功结果，减少重复审校造成的等待和 token 消耗；临时不可用的接收闸门结果不会被缓存。
- 手动章节生成、旧兼容入口和批量执行会进入同一条章节生产链，减少不同入口使用不同生成策略导致的章节状态、质量修复和恢复判断差异。
- 服务端和桌面端会自动清理过期的文件日志，LLM 调试日志和桌面主日志过大时会自动轮转；清理只处理已知日志后缀，不会触碰数据库、小说数据、图片、备份或未知文件。
