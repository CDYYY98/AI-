...
角色准备、章节生产、卷规划同步、后台副作用、桌面打包、图片生成厂商和全书自动执行链路更稳：用户在角色准备页应用一套阵容后，核心角色、关系和方案状态会先同步完成；卷规划同步到章节执行区时，会记住每个规划章节对应的真实执行章节；自定义 OpenAI 兼容厂商也可以配置图片模型并参与角色形象图生成；自动导演遇到需要确认的角色阵容候选时，会停在可处理的审核点；全书自动执行遇到重复质量问题时，也能更准确暂存当前章节风险并继续推进；空白章节会按可重新写作的状态提示处理，已登记为质量待回收的章节也能继续后续章节。

- 应用角色阵容后会提示外显资料和角色动态正在后台补齐，用户可以先继续进入角色资产工作台。
- 从角色准备页或自动导演应用阵容时，后台补齐会沿用当前选择的模型参数，减少应用阵容时长时间等待、结果偏离所选模型或误以为页面卡住的情况。
...
- 章节进度会优先按正文、审校问题和真实生成状态判断，减少空章节被当成完成、已修复章节继续显示待修复的情况。
- 自动导演恢复章节执行时会重新读取最新章节事实，旧任务里残留的失败状态不会覆盖已经生成完成的正文。
- 关闭自动审校的自动执行会按“计划内跳过审校”继续提交章节，不会因为没有审校报告把已经写完的章节卡住。
- 遇到重规划或质量修复建议时，可以选择先跳过本次建议并继续后续章节；系统会把当前章节登记为质量待回收，避免自动导演在同一个质量门口反复停住。
- 章节接收闸门会区分本章必须完成的硬性义务和可后续回收的轻微缺口；轻微伏笔、角色触碰或目标变化缺口会登记为风险继续，不再让章节反复卡在局部修复。
- 已经细化过任务单和场景卡的章节，在没有新修改要求时会直接沿用已有规划，减少章节执行前重复等待 AI 重新细化。
- 任务中心查看自动导演任务时，会优先显示实时任务和后台执行器合成后的状态；旧的等待确认或失败快照不会再覆盖正在运行、排队或恢复中的真实进度。
- 自动导演进度弹窗会优先相信正在运行的真实进度；旧的等待确认或阻塞状态不会再把正在细化、写作或审校的任务误显示成等待确认。
- 小说页接管提示、进度弹窗、任务抽屉和侧边驾驶舱会统一读取同一套自动导演展示状态，主进度、当前动作和按钮不会再由多个前端入口各自判断。
- GitHub Actions 桌面打包流程改用 Node 24 运行环境验证，提前适配新版 Actions 运行时，减少后续生成安装包时因 Node 版本切换失败的风险。
- 后台模型管理可以为自定义厂商填写图像模型，角色形象图生成可以使用自己的 OpenAI 兼容中转或本地网关。
- 角色形象图弹窗会按后台配置列出已启用、已配置且填写了图像模型的厂商，避免用户选到无法生成图片的通道。
- 自定义图片厂商支持不填写 API Key，适合本地网关或不需要鉴权的中转服务。
- 章节写作会用本章目标、必须推进事项、冲突和出场角色生成知识库检索问题，让正文生成拿到的参考资料更贴近当前章节任务。
- 世界观结构化手册会从旧版文本和 AI 生成 JSON 中提取势力、地点、关键冲突和控制关系，减少世界观资料只停留在大段文本里、后续角色和章节无法稳定引用的情况。
- 世界观保存时会把结构化信息同步回背景、规则、文化、力量体系、地理、冲突、历史和经济等旧字段，兼容现有页面和生成链路。
- 世界观可视化里的地理视图会按相对方位绘制地点、路线、风险和叙事作用，AI 抽取可直接补充坐标、地形、地点类型和路线类型，用户查看世界地图时更容易判断舞台位置、通路关系和剧情压力点。
- 世界观结构补全会正确接收“阵营 + 具体势力”的组合结果，减少生成势力结构时被格式校验误拦截的情况。
- 后台模型配置页读取厂商状态时只加载本地保存的模型和内置候选，不再自动访问远程模型目录；本机中转、内网网关或外部厂商暂时不可达时，打开设置页也不会被探测请求拖慢。
- 知识库文档归档后会移出默认检索和资料选择，但保留原文与版本；从“仅归档”恢复启用时会自动重新排队建索引，恢复完成后可继续召回测试和创作引用。

### 2026-06-07

章节生成后的结构化审校、自动导演接管状态、运行展示和修复兜底更稳：系统会更明确约束 AI 输出格式，更准确地区分真正的自动导演任务和普通编辑任务，并按当前执行范围判断章节审校与修复进度。
...
- 个人中心也会显示“软件更新”入口，普通用户不用进入管理员设置页就能检查客户端版本。
- 便携版保留为特殊场景的手动替换方式，不进入正式用户的自动更新链路。
