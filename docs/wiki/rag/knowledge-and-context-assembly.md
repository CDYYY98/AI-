...
- 归档知识库文档只表示“移出默认检索和资料选择”，不是删除原文或版本。归档时应排队删除向量分块并把索引状态回到 `idle`；恢复启用时如果有激活版本，必须把索引状态设为 `queued` 并排队重建，之后才能重新召回测试或参与创作引用。
- 章节写作阶段的检索 query 应优先使用本章任务信息，包括章节目标、预期效果、必须推进事项、目标冲突和出场角色；只有缺少这些运行时信号时，才回退到章节序号、标题和结构化大纲摘要。
