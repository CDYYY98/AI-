...
- 全书自动接管可以把章节任务单细化延后到章节执行前自动生成。结构化大纲阶段只要卷拆章列表已经就绪，就不应因为 `chapterDetailReady=false` 把 `chapter_detail_bundle` 校验为失败；普通分阶段规划仍应按完整章节任务单事实判断完成度。
...
- 自动导演事实汇总、章节执行步骤校验、进度展示和恢复提示必须重新读取最新章节进度，再按当前自动执行范围裁剪；不得复用 `state.chapterProgress` 里的旧快照作为完成或失败判断。旧任务状态可以作为诊断材料，但不能覆盖已经落库的正文、审校和状态提交事实。
- 章节状态提交事实也必须按当前自动执行范围裁剪。`chapter.state.commit` 只能统计本轮范围内已保存正文和已提交状态的章节；范围外未提交的旧章节不得让当前批次停在未完成状态。
- 当最新章节事实显示当前范围没有任何可保存正文时，章节执行校验应优先暴露 `lastError` 或 `checkpointSummary` 中的真实中断原因。空正文判断本身可以阻止完成，但不能用通用“没有正文”文案盖住模型异常、额度熔断、重规划暂停或人工确认点。
