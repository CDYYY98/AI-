...
const {
  registerNovelEventHandlers,
} = require("../dist/events/handlers/registerNovelEventHandlers.js");
...
test("pipeline completed event enqueues a snapshot side effect instead of executing inline work", async () => {
  ...
  registerNovelEventHandlers(eventBus, {
    sideEffectJobs: {
      async enqueueJob(input) {
        enqueued.push(input);
        return { job: { id: "job-enqueued" }, created: true };
      },
    },
  });
  ...
  await eventBus.emit({
    ...
    payload: {
      novelId: "novel-1",
      jobId: "pipeline-123456789",
      status: "succeeded",
    },
  });

  assert.equal(enqueued.length, 1);
  assert.equal(enqueued[0].jobType, "novel.pipelineSnapshot");
  assert.equal(enqueued[0].idempotencyKey, "novel.pipelineSnapshot:pipeline-123456789");
