...
export class NovelSideEffectJobService {
  ...
  async markFailedOrDead(
    job: NovelSideEffectJob,
    error: unknown,
    options?: { forceDead?: boolean },
  ): Promise<NovelSideEffectJobStatus> {
    const nextStatus: NovelSideEffectJobStatus = options?.forceDead || job.attempts >= job.maxAttempts
      ? "dead"
      : "failed";
