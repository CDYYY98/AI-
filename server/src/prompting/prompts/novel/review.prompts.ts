...
> = {
  ...
  render: (input) => [
    new SystemMessage([
      ...
      "",
      "【concreteFacts 抽取要求】",
      "concreteFacts 用于记录本章正文已经写明、后续章节必须保持一致的硬事实。请逐条抽取，每条不超过 40 字：",
      "1. 关键角色做出的承诺、约定、交易条款，尽量带上金额、数量、时间、地点或方式。",
      "2. 本章确立的事件性质，尤其是私下行为、公开行为、官方流程、秘密交易等不可随意改写的属性。",
      "3. 关键数字、日期、场次、票据编号、人物身份、关系状态等硬细节。",
      "4. category 取值只能是：completed=本章已完成的过程性目标；revealed=本章已揭示的信息或秘密；state_changed=关系、状态、约定或交易的变化。",
      "5. 只抽取正文真实写到的具体事实，不得臆造；本章若无明确硬事实，concreteFacts 可为空数组。",
      "6. 不要把抽象目标或主题口号写进 concreteFacts，只记可能被后文写错的具体事实。",
      "",
      "【质量要求】",
