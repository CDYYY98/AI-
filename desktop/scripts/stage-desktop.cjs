...
function writeDeployConfig() {
  ...
  const deployConfigPath = path.join(resourcesDir, "deploy.json");
  const config = JSON.stringify({ apiBaseUrl: apiBaseUrl || null }, null, 2);
  fs.writeFileSync(deployConfigPath, config, "utf8");
  if (apiBaseUrl) {
    console.log(`[stage:desktop] deploy.json written: ${apiBaseUrl}`);
  } else {
    console.log("[stage:desktop] deploy.json written (no remote API configured).");
  }
}
