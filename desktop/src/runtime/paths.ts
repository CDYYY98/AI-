...
const APP_NAME = "Turing-Novel-Workbench";
...
function resolveDeployConfig(): { apiBaseUrl?: string } | null {
  try {
    const configPath = path.join(resolveDesktopResourcesDir(), "deploy.json");
    if (fs.existsSync(configPath)) {
      return JSON.parse(fs.readFileSync(configPath, "utf-8"));
    }
  } catch {
    // Ignore missing or invalid deploy config.
  }
  return null;
}
...
export function resolveDesktopRuntimeConfig(options: {
  ...
  const remoteApiBaseUrl = deployConfig?.apiBaseUrl?.trim()
    || process.env.AI_NOVEL_API_BASE_URL?.trim();
  const apiBaseUrl = remoteApiBaseUrl
    ? remoteApiBaseUrl.replace(/\/+$/, "")
    : `http://127.0.0.1:${options.port}/api`;
