...
function isRemoteApiConfigured(): boolean {
  if (process.env.AI_NOVEL_API_BASE_URL?.trim()) {
    return true;
  }
  try {
    const resourcesDir = process.resourcesPath || resolveDesktopResourcesDir();
    const deployPath = path.join(resourcesDir, "deploy.json");
